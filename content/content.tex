...
\begin{table}[]
    ...
    \begin{tabular}{c|c|c}
        ...
        \hline
        & \\info(), info(expId) & GET & /BaseURL:port/RIP \\
        \hline
        & \\set(expId, variablenames, variablevalues), & POST & /BaseURL:port/RIP/POST
          \\get(expId, variablenames) &  & \\
        \hline
        & \\connect(expId) & SSE & /BaseURL:port/RIP/SSE \\
        \hline
    \end{tabular}
...
A RIP client must implement a class with the methods described in Section \ref{sec:External_Functions}. Next, detailed information of each of them is given. This information includes a description of: 1) the method itself, its input parameters and the result returned by the method. 
...
When called with the \textit{expId} parameter, \textbf{info(expId)} returns meta-data of the referenced \textit{experience}.

The result returned by this method is... TODO. %Jesus - En este caso, la información devuelta es justamente la que se detalla y explica en la sección de Developer Documentation, pero aquí habría que intentar hablar de ellos desde un punto de vista del usuario solamente.
...
The result returned by this method is... TODO. %Jesus - Habría que explicar que las variables deben ser enlazadas antes para que se produzca la actualización de valores automática de las mismas... para esto debe existir otro método que usa el cliente rip y que también habría que llamar (en nuestra implementación de EjsS, se hace a través de una GUI, pero podría no ser así). Habría que incluir ese método como subsection aquí y en la sección 2.2.2. Otro método que creo que también deberíamos incluir es el que se usa para configurar el elemento de EjsS, donde se indica la IP del servidor y el puerto.

\subsection{Method set}
This method has three input parameters: \textit{expId}, \textit{VariablesName} and \textit{VariablesValue}.

When this method is called, the values specified in \textit{VariablesValue} are assigned to the \textit{control program's} or \textit{simulation model's} variables named as the strings in \textit{VariablesName}. The \textit{control program} or \textit{simulation model} is determined with the \textit{expId} parameter. 

The result returned by this method is... TODO. %Jesus - ¿Qué devuelve el método rip.set() de EjsS?

\subsection{Method get}
This method has two input parameters: \textit{expId} and \textit{VariablesName}.

When this method is called, the values of the \textit{control program's} or \textit{simulation model's} variables named as the strings in \textit{VariablesName} are read and returned. The \textit{control program} or \textit{simulation model} is determined with the \textit{expId} parameter.

The result returned by this method is... TODO. %Jesus - ¿Qué devuelve el método rip.get() de EjsS exactamente?
...
\begin{table}[b!]
    ...
    \begin{tabular}{|c|c|}
        ...
        & \\info() & basicInfo() \\
        \hline
        & \\info(expId) & experienceInfo() \\
        \hline
        & \\connect(expId) & 1. - start(), 2. - get() \\
        \hline
        & \\set(expId, VariablesName, VariablesValue) & set() \\
        \hline
        & \\get(expId, VariablesName) & get() \\
        \hline
    \end{tabular}
...
\begin{table}[b!]
    ...
    \begin{tabular}{|c|c|}
        ...
        \hline
    \end{tabular}
    \caption{Triggering conditions and fired internal method}
...
This method is called only when the RIP server receives an HTTP message from a client with the \textit{get} method and the \textit{expId} and \textit{VariablesName} parameters.
...
This method is called only when the RIP server receives an HTTP message from a client with the \textit{set} method and the \textit{expId}, \textit{VariablesName} and \textit{VariablesValue} parameters.

It writes the values received in the specified variables of the appropiate OL \textit{experience}.
...
The \textit{info()} RIP client method sends a GET request to \url{http(s)://BaseURL:port/RIP}, while \textit{info(expIdValue)} does the same with \url{http(s)://BaseURL:port/RIP?expId=expIdValue}.
...
The \textit{info()} RIP client method sends a GET request to: \url{http://10.192.38.56:8080/RIP}
...
The \textit{info(expId)} RIP client method sends a GET request to: \url{http://10.192.38.56:8080/RIP?expId=Test1}
...
The \textit{connect(expIdValue)} RIP client method establishes an SSE connection with: \url{http://10.192.38.56:8080/RIP/SSE?expId=expIdValue}.
...
The \textit{set(expIdValue, VariablesNames, VariablesValues)} RIP client method sends a POST request to: \url{http(s)://BaseURL:port/RIP/POST?expId=expIdValue} with a JSON body with the following top-level members:
...
\begin{myEnumerate}
    ...
    \begin{myEnumerate}
        ...
        \item \textbf{VariablesName}: An array of strings with the name of the variables in the \textit{control program} or \textit{simulation model} whose values are going to be overwritten.
        \item \textbf{VariablesValue}: A mixed array with the values to write in the \textit{control program's} or \textit{simulation model's} variables.
    \end{myEnumerate}
    \item \textbf{id}: A string with a number that indicates how many request have been sent till now, including this one.
\end{myEnumerate}
...
The \textit{set(expIdValue, VariablesNames)} RIP client method sends a POST request to: \url{http(s)://BaseURL:port/RIP/POST?expId=expIdValue} with a JSON body with the following top-level members:
...
\begin{myEnumerate}
    ...
    \begin{myEnumerate}
        \item \textbf{expId}: A string with the \textit{experience} identifier.
        \item \textbf{VariablesName}: An array of strings with the name of the variables in the \textit{control program} or \textit{simulation model} whose values are going to be retrieved.
    \end{myEnumerate}
    \item \textbf{id}: A string with a number that indicates how many request have been sent till now, including this one.
\end{myEnumerate}
